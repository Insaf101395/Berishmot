...
async def process_category(callback: types.CallbackQuery, state: FSMContext):
    ...
    if category == "main_only":
        # Skip warehouse step — set a default warehouse value to avoid KeyError
        await state.update_data(warehouse="Доставка 2-5 дней")
...
async def send_preview(message: types.Message, state: FSMContext):
    ...
    warehouse = data.get("warehouse", "Доставка 2-5 дней")
    category = data.get("category", "main_only")
